...
Begin VB.Form frmEditor_MapProperties 
   ...
   Begin VB.Frame fraMapSettings 
      ...
      Begin VB.ComboBox cmbMoral 
         ...
         Left            =   960
         List            =   "frmMapProperties.frx":004F
         Style           =   2  'Dropdown List
         TabIndex        =   14
         Top             =   360
         Width           =   3135
...
Private Sub cmdOk_Click()
    ...
    With Map
        ...
        .Intensity = scrlWeatherIntensity
        .LevelPoke(1) = scrlLevel(0).value
        .LevelPoke(2) = scrlLevel(1).value
...
Private Sub scrlLevel_Change(Index As Integer)
    Select Case Index
    Case 0
        Map.LevelPoke(1) = scrlLevel(Index).value
        lblLevel(Index).Caption = "Min: " & scrlLevel(Index).value
    Case 1
        Map.LevelPoke(2) = scrlLevel(Index).value
        lblLevel(Index).Caption = "Máx: " & scrlLevel(Index).value
    End Select
End Sub

Private Sub scrlWeatherIntensity_Change()
    Map.Intensity = scrlWeatherIntensity.value
